...
export default function SeeAllTopRated() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}getToprated`,
        {
          page: page,
        }
      );
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      {
        threshold: 0, // Reagiert, sobald 10% des Elements sichtbar sind
      }
    );
      ...
      >
        <div className="w-full h-full flex items-end justify-between px-6 py-4">
          ...
          <div className="absolute top-0 left-0 w-full h-full px-6 py-4 flex items-end justify-center pointer-events-none">
            <h2 className="text-2xl font-light">Bestbewertete Filme</h2>
          </div>
        </div>
      </div>
      {isInitialLoading ? (
